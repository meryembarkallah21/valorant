...
# Assign the driver path directly in the script
DRIVER_PATH = "C:\\Users\\Extreme PC\\chromedriver-win64\\chromedriver.exe"

response = requests.get('https://blitz.gg', verify=False)
...
class valorantBot():
    ...
    def extractMapsFromRank(self, maxMaps = 6):
        numMaps = 0
    ...
    def extractMaps(self, maxMaps = 6, modes = ["Competitive", "Unrated", "Spike Rush", "Custom"]):
        maps_list = []


        # unrated spikerush custom -tier #0 #2 #3
        # Iron 1 - tier #3
        # Immortal - tier #21
        # Radiant - tier #24
        ...
        for mode in modes:
            ...
            if mode == "competitive":
                for i in range(3, 22):
                    ...
                    self.driver.get(f'https://blitz.gg/valorant/stats/maps?map=all&act=e7act3&queue=competitive&tier={i}')
                    sleep(1)
                    maps_df = self.extractMapsFromRank()
                    maps_df.to_csv(f"map_data/maps_competitive_tier={i}.csv")
            else:
                self.driver.get(f'https://blitz.gg/valorant/stats/maps?map=all&act=e7act3&queue={mode}&tier=0')
                sleep(1)
                maps_df = self.extractMapsFromRank()
                maps_df.to_csv(f"map_data/maps_{mode}_tier=0.csv")

    def extractAgentsFromRank(self, maxAgents = 15):
        agents_list = []
    ...
    def extractAgents(self, modes = ["Competitive", "Unrated", "Spike Rush", "Custom"], maps = ["All", "Bind", "Split",
        ...
        for mapName in maps:
            ...
            for mode in modes:
                ...
                if mode == "competitive":
                    for i in range(3, 22):
                        print(f"Completing analysis of tier {i}...")
                        self.driver.get(f'https://blitz.gg/valorant/stats/agents?map={mapName}&act=e7act3&queue=competitive&tier={i}')
                        sleep(1)
                        agents_df = self.extractAgentsFromRank()
                        agents_df.to_csv(f"agents_data/{mapName}/agents_competitive_tier={i}.csv")
                else:
                    self.driver.get(f'https://blitz.gg/valorant/stats/agents?map={mapName}&act=e7act3&queue={mode}&tier=0')
                    sleep(1)
                    agents_df = self.extractAgentsFromRank()
                    agents_df.to_csv(f"agents_data/{mapName}/agents_{mode}_tier=0.csv")

    def extractAbilitiesFromRank(self, maxAgents = 15):
    ...
    def extractAbilities(self, modes = ["Competitive", "Unrated", "Spike Rush", "Custom"], maps = ["All"]):
        for mapName in maps:
            mapName = mapName.lower()
            ...
            for mode in modes:
                ...
                if mode == "competitive":
                    for i in range(3, 22):
                        ...
                        self.driver.get(f'https://blitz.gg/valorant/stats/agents?map={mapName}&act=e7act3&queue=competitive&tier={i}')
                        sleep(1)
                        agents_df = self.extractAbilitiesFromRank()
    ...
    def extractWeaponsFromRank(self, maxWeapons = 17):
        numWeapons = 0
    ...
    def extractWeapons(self, modes = ["Competitive", "Unrated", "Spike Rush", "Custom"], maps = ["All", "Bind", "Split",
        "Ascent", "Haven", "Icebox", "Breeze"]):
        ...
        for mapName in maps:
            ...
            for mode in modes:
                ...
                if mode == "competitive":
                    for i in range(3, 22):
                        print(f"Completing analysis of tier {i}...")
                        self.driver.get(f'https://blitz.gg/valorant/stats/weapons?map={mapName}&act=e7act3&queue=competitive&tier={i}')
                        sleep(1)
                        agents_df = self.extractWeaponsFromRank()
                        agents_df.to_csv(f"weapons_data/{mapName}/agents_competitive_tier={i}.csv")
                else:
                    self.driver.get(f'https://blitz.gg/valorant/stats/weapons?map={mapName}&act=e7act3&queue={mode}&tier=0')
                    sleep(1)
                    agents_df = self.extractWeaponsFromRank()
...
bot = valorantBot()
bot.extractMaps()
bot.extractAgents(maps=["All"])
bot.extractAbilities(maps=["All"])
bot.extractWeapons(maps=["All"])